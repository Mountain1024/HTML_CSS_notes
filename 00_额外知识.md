...
- **组件复用**：将按钮、卡片等重复元素封装为可复用组件，可通过 CSS 类或前端框架（如 React、Vue）实现。
...
```html
ul>li.item$*5
<!-- 自动生成 li.item1、li.item2...li.item5 -->
```
...
1. **定义尺寸**：如图标 20x20px，设 `width: 20px; height: 20px;`。

2. **引入图片**：`background-image: url(sprite.png);`。
3. **定位显示**：`background-position: -20px -40px;`。
...
随着 HTTP/2 的普及和 SVG 的广泛支持，精灵图使用正在减少，可考虑：
...
@font-face {
  ...
  src: url("./fonts2/AaJianHaoTi.eot?#iefix") format("embedded-opentype"), url("./fonts2/AaJianHaoTi.woff")
      format("woff"), url("./fonts2/AaJianHaoTi.ttf") format("truetype"), url("./fonts2/AaJianHaoTi.svg#uxfonteditor")
      format("svg");
}
```
