...
**Web Fonts** 允许网页使用非系统字体，通过浏览器下载指定的字体文件来渲染文本
...
将多个小图标合并为一张大图，通过 `background-position` 控制显示位置，以减少 HTTP 请求并提升页面性能。
...
- **字体图标**：可调整大小和颜色，兼容性较好。
- **CSS 绘制**：使用 `border`、`clip-path` 等绘制简单图标。
...
字符实体是 HTML 中用于表示特殊字符的编码格式，以 `&` 开头、`;` 结尾，常用于在 HTML 文本中显示保留字符或特殊符号。例如：
...
**URL（统一资源定位符，Uniform Resource Locator）** 是互联网上定位和访问资源的标准方式，相当于网络中的“门牌号”，能够唯一指向特定资源。
...
- **`<协议>`**：定义访问资源的网络协议，如 `HTTP`（超文本传输协议）、`HTTPS`（安全 HTTP）、`FTP`（文件传输协议）。
- **`<域名或IP地址>`**：资源的网络位置，例如 `www.example.com` 或 IP 地址 `192.168.1.1`。
- **`<端口号>`**：服务器提供资源的服务端口，默认值（如 HTTP 的 `80`、HTTPS 的 `443`）可省略。
- **`<文件路径><文件名>`**：资源在服务器上的具体位置，如 `/index.html` 或 `/images/logo.png`。
- **`<查询参数>`**：传递给服务器的额外信息，通常为键值对，如 `key1=value1&key2=value2`，常用于搜索或动态内容。
- **`<锚点>`**：资源内的特定位置，如 `#section1`，用于跳转到页面中的某个部分。
