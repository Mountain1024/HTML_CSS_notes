...
```css
.icon { width: 20px; height: 20px; background: url(sprite.png) -20px -40px no-repeat; }
```
...
> - **SVG 优势**：支持多色、动态调整大小、文件轻量、无需HTTP请求（内联使用）。  
>- **建议**：现代项目优先考虑 SVG，精灵图适用于传统场景。

## 四、Web Fonts

字体文件与 HTML/CSS/JavaScript 一同部署于静态服务器。
...
- **`<协议>`**：指定访问资源时使用的网络协议，例如 `HTTP`（超文本传输协议）、`HTTPS`（安全的HTTP）或 `FTP`（文件传输协议）。
